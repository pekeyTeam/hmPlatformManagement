            ...
            <el-col :span="12">
              <el-form-item label="聘用形式" prop="formOfEmployment">
                <el-select v-model="userInfo.formOfEmployment" size="mini" class="inputW">
                  <el-option label="正式" :value="1">正式</el-option>
                  <el-option label="非正式" :value="2">非正式</el-option>
                </el-select>
              </el-form-item>
            </el-col>
          </el-row>
...
import { addEmployeeService, getEmployeeDetailService, updateEmployeeDetailService } from '@/api/employee'
export default {
  components: {
    SelectTree,
    ImageUpload
  ...
  methods: {
    async saveData() {
      ...
      if (this.$route.params.id) {
        // 修改
        await updateEmployeeDetailService(this.userInfo)

        this.$message.success('修改成功')
      } else {
        // 新增

        await addEmployeeService(this.userInfo)
    ...
    async getEmployeeDetail() {
      const res = await getEmployeeDetailService(this.$route.params.id)
      // console.log(res)
      this.userInfo = res
    }
  }
