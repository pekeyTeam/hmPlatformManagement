...
import { getHomeDataService, getNoticeListService } from '@/api/home'

// import * as eCharts from 'echarts'// 引入所有的Charts
...
export default {
  ...
  computed: {
    ...mapGetters(['company', 'name', 'departmentName', 'avatar'])
  },
  watch: {
    homeData() {
  ...
  mounted() {
    this.myChart1 = eCharts.init(this.$refs.social)
    this.myChart2 = eCharts.init(this.$refs.provident)

    // console.log(myChart)
  },
  methods: {
    async getHomeData() {
      this.homeData = await getHomeDataService()
    },

    async getNoticeList() {
      this.list = await getNoticeListService()
    }
  }
}
</script>
